...
void HistFactDstTauDemo(TString inputDir, TString outputDir) {
  ...
  double mcN_sigmu,mcN_sigtau,
    mcN_D1;
  double *mcnorms[3]={&mcN_sigmu,&mcN_sigtau,
                      &mcN_D1};
  for(int i =0; i < 3; i++){
  ...
  */
  ...
  if (toyMC)
    {
      ...
      sw2.Stop();
      RooFormulaVar poi_pull("poi_pull","title", "(@0-0.03428)/@1",RooArgList(*poi,poierror));
  ...
  for (int i = 0; i < nframes; i++){
    ...
    model_hf->plotOn(drawframes[i], Slice(*idx),ProjWData(*idx,*data),DrawOption("F"),FillColor(kViolet),Components("*misID*,*sigmu*,*D1*"));
      ...
      for (int k = 0 ; k < q2_bins*2; k++)
        {
          ...
          if(k >= q2_bins) {t->DrawLatex(2250,q2frames[k]->GetMaximum()*0.92,"Demo");}
          if(k < q2_bins) {t->DrawLatex(8.7,q2frames[k]->GetMaximum()*0.92,"Demo");}
